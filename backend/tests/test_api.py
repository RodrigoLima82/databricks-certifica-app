...
def test_register_senha_curta_422(client):
    r = client.post("/api/auth/register", json={"name": "X", "email": "x@example.com", "password": "123"})
    assert r.status_code == 422


def test_login_errado_401(client):
    client.post("/api/auth/register", json={"name": "Y", "email": "y@example.com", "password": "senha123"})
    r = client.post("/api/auth/login", json={"email": "y@example.com", "password": "errada"})
    assert r.status_code == 401


def test_email_duplicado_409(client):
    client.post("/api/auth/register", json={"name": "Z", "email": "z@example.com", "password": "senha123"})
    r = client.post("/api/auth/register", json={"name": "Z2", "email": "z@example.com", "password": "senha123"})
